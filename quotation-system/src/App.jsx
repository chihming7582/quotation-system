...
/* ─── Storage ─── */
const CLIENT_KEY = "qs_clients_v2";
const QNUM_KEY   = "qs_qnum_v2";

const loadClients = () => { try { return JSON.parse(localStorage.getItem(CLIENT_KEY) || "[]"); } catch { return []; } };
const saveClients = (c) => localStorage.setItem(CLIENT_KEY, JSON.stringify(c));
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin:0; padding:0; }
  ...
  :root {
    --bg:#0b0f1a; --surface:#111827; --card:#1a2236;
    --border:#1e2d45; --accent:#e8a020; --text:#f0f4ff;
...
/* ─── Quote Preview (used in both desktop right panel & print) ─── */
  ...
  return (
    <div style={{background:"white",color:"#1a1a1a",fontFamily:"'Noto Sans TC','Microsoft JhengHei',sans-serif",fontSize:12,lineHeight:1.6}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20,paddingBottom:16,borderBottom:"3px solid #1d3461"}}>
        <div>
          <div style={{fontSize:22,fontWeight:800,color:"#1d3461",letterSpacing:-0.5}}>{form.companyName||"公司名稱"}</div>
      ...
      <div style={{display:"flex",justifyContent:"flex-end",marginBottom:16}}>
        <div style={{minWidth:220}}>
          <div style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid #e5e7eb",fontSize:12}}>
            <span style={{color:"#666"}}>小計</span><span style={{fontWeight:500}}>{fmt(subtotal)}</span>
          </div>
          {(+form.taxRate)>0&&<div style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid #e5e7eb",fontSize:12}}>
            <span style={{color:"#666"}}>稅金（{form.taxRate}%）</span><span style={{fontWeight:500}}>{fmt(tax)}</span>
          </div>}
...
export default function App() {
  ...
  const saveClient = () => {
    if(!form.clientName&&!form.clientCompany) { showToast("⚠️ 請先填入客戶姓名或公司"); return; }
    const existing = clients.find(c=>c.company===form.clientCompany&&c.name===form.clientName);
    let updated;
    if(existing) {
      updated = clients.map(c=>c.id===existing.id?{...c,name:form.clientName,company:form.clientCompany,address:form.clientAddress,phone:form.clientPhone,email:form.clientEmail}:c);
      showToast("✅ 客戶資料已更新");
    } else {
      const nc = {id:Date.now().toString(),name:form.clientName,company:form.clientCompany,address:form.clientAddress,phone:form.clientPhone,email:form.clientEmail};
      updated = [nc,...clients];
      setSelClient(nc.id);
      showToast("✅ 已儲存新客戶");
    }
  ...
  const FormPanel = () => (
    <div style={{padding: isMobile ? "14px 14px 100px" : "20px", overflowY:"auto", height:"100%"}}>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        {clients.length===0&&<div style={{fontSize:11,color:"var(--muted)",marginTop:8,textAlign:"center"}}>尚無儲存的客戶，填入後點「儲存客戶」</div>}
      </div>
      ...
      {formSection==="client"&&<>
        ...
        <button onClick={saveClient} className="btn btn-success" style={{width:"100%",padding:"11px",fontSize:13,marginTop:4}}>
          💾 儲存此客戶資料
        </button>
        {clients.length>0&&<>
      ...
      {formSection==="items"&&<>
        ...
        {form.items.map((item,idx)=>(
          <div key={item.id} className="card" style={{marginBottom:12}}>
            ...
            <div style={{textAlign:"right",fontSize:12,color:"var(--sub)",marginBottom:8}}>
              小計：<span style={{color:"var(--accent)",fontWeight:700}}>{fmt((+item.qty||0)*(+item.unitPrice||0))}</span>
            </div>
            <Inp label="備註（選填）"><input className="inp" placeholder="此項備註" value={item.note} onChange={e=>setItem(idx,"note",e.target.value)}/></Inp>
          </div>
        ))}
        <button onClick={addItem} className="btn btn-ghost" style={{width:"100%",padding:"11px",fontSize:13,borderStyle:"dashed"}}>
          ＋ 新增項目
        </button>
        <div className="card" style={{marginTop:14,background:"#0d1a2d"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",fontSize:15,fontWeight:800,color:"var(--accent)"}}>
            <span>總計</span><span>{fmt(total)}</span>
          </div>
        </div>
      </>}
      ...
      <div className="qs-root">
        ...
        <div style={{background:"#111827",borderBottom:"1px solid #1e2d45",padding:isMobile?"12px 14px":"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              ...
              {!isMobile&&<div style={{fontSize:10,color:"var(--muted)"}}>Quotation Management System</div>}
            </div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={newQuote} className="btn btn-ghost" style={{padding:isMobile?"7px 10px":"8px 14px",fontSize:12}}>
              {isMobile?"＋":"＋ 新報價單"}
            </button>
            <button onClick={handlePrint} className="btn btn-primary" style={{padding:isMobile?"7px 12px":"8px 16px",fontSize:12}}>
              {isMobile?"⬇ PDF":"⬇ 匯出 PDF"}
            </button>
          </div>
        </div>
        ...
        {!isMobile&&(
          <div style={{display:"flex",height:"calc(100vh - 65px)"}}>
            <div style={{width:"46%",minWidth:340,borderRight:"1px solid var(--border)",overflow:"hidden",display:"flex",flexDirection:"column"}}>
              <FormPanel/>
            </div>
            <div style={{flex:1,overflowY:"auto",background:"#0d1117",padding:20}}>
              ...
              <div style={{background:"white",borderRadius:10,padding:"28px 32px",boxShadow:"0 8px 40px rgba(0,0,0,.5)"}}>
                <QuotePreview form={form}/>
              </div>
            </div>
          </div>
        ...
        {isMobile&&(
          <>
            <div style={{height:"calc(100vh - 57px)",overflowY:"auto"}}>
              ...
              {mobileTab==="preview"&&(
                <div style={{padding:"14px 14px 100px",background:"#0d1117",minHeight:"100%"}}>
                  ...
                  <div style={{background:"white",borderRadius:10,padding:"16px",boxShadow:"0 8px 40px rgba(0,0,0,.5)"}}>
                    <QuotePreview form={form}/>
                  </div>
                  <button onClick={handlePrint} className="btn btn-primary" style={{width:"100%",marginTop:16,padding:14,fontSize:14}}>
                    ⬇ 匯出 PDF
                  </button>
                </div>
              )}
              ...
              {mobileTab==="clients"&&(
                <div style={{padding:"14px 14px 100px"}}>
                  <div className="section-title">客戶資料庫</div>
                  <div className="card" style={{marginBottom:14}}>
                  ...
                  {clients.map(c=>(
                    <div key={c.id} className="card" style={{marginBottom:10}}>
                      ...
                      <button onClick={()=>{ selectClient(c.id); setMobileTab("form"); showToast(`✅ 已填入 ${c.company||c.name}`); }} className="btn btn-ghost" style={{width:"100%",marginTop:10,padding:"8px",fontSize:12}}>
                        選用此客戶 →
                      </button>
                    </div>
                  ))}
            ...
            <nav className="mob-nav">
              {[
                {k:"form",    icon:"✏️", label:"填寫"},
                {k:"preview", icon:"👁️", label:"預覽"},
                {k:"clients", icon:"👥", label:`客戶${clients.length>0?`(${clients.length})`:""}`},
              ].map(({k,icon,label})=>(
